...
def standardize_columns(df):
    ...
    column_mapping = {
        # 凭证相关（仅财务凭证号，不要和包裹单号混淆）
        '凭证号': '凭证', '单据号': '凭证', '凭证编号': '凭证',
        # 包裹号相关（快递/运单/订单单号 = 包裹号，是“日期+包裹号”去重主键）
        '单号': '包裹号', '快递单号': '包裹号', '运单号': '包裹号', '订单号': '包裹号',
        '物流单号': '包裹号', '包裹单号': '包裹号', '件号': '包裹号', '运单': '包裹号',
        # 日期相关（精确 + 常见业务变体）
    ...
    # ── 智能兜底：包裹号 ──
    if '包裹号' not in df.columns:
        for col in df.columns:
            if any(kw in str(col) for kw in ['包裹', '单号', '运单', '快递', '件号']) \
               and '凭证' not in str(col):
                df = df.rename(columns={col: '包裹号'})
                break  # 只映射第一个匹配到的列
